...
var SqlTree = Class.extend({
  ...
  populateInclude: function (result, item, include) {
    ...
    for (i = 0; i < groups.length; i++) {
      ...
      includeItemInstance = this.toObject(includeItem, includeItemInstance, include.tableAlias);
  ...
  toObjects: function (rows) {
    ...
    if (this.primaryKeyColumn) {
      ...
      for (i = 0; i < items.length; i++) {
        ...
        result = this.toObject(item[0], result, this.primaryKeyColumn.tableAlias);
        results.push(result);
  ...
  toObject: function (row, result, alias) {
    var key;
    ...
    for (key in row) {
      ...
      if (alias && alias !== column.tableAlias) {
        continue;
      }
