...
var Connection = Class.extend({
  ...
  _save: function(isNew, obj, callback) {
    ...
    function doTypeConversions() {
      for (var columnKey in obj._getModel().columns) {
        var columnInfo = obj._getModel().columns[columnKey];
        if (obj[columnKey] && columnInfo.type === type.JSON) {
          if (typeof obj[columnKey] === 'object') {
            obj[columnKey] = JSON.stringify(obj[columnKey]);
          }
        }
      }
    }

    function doEventFn(name, callback) {
      if (obj._getModel()[name]) {
        return obj._getModel()[name](obj, self, callback);
      }
      else {
        return callback();
      }
    ...
    function doSave() {
      ...
      tasks.push(function(callback) {
        doEventFn('onSave', callback);
      });

      tasks.push(function(callback) {
        obj._getModel().emit('beforeSave', obj);
        doEventFn('beforeSave', callback);
      });

      tasks.push(function(callback) {
        if (isNew) {
          obj._getModel().emit('beforeCreate', obj);
          doEventFn('beforeCreate', callback);
        } 
        else {
          obj._getModel().emit('beforeUpdate', obj);
          doEventFn('beforeUpdate', callback);
        }
      });
      ...
      tasks.push(function(data, callback) {
        ...
        self.saveAssociations(obj, function(err, savedObj) {
          if (isNew) {
      ...
      tasks.push(function(callback) {
        if (isNew) {
          doEventFn('afterCreate', callback);
        } 
        else {
          doEventFn('afterUpdate', callback);
        }
      });

      tasks.push(function(callback) {
        doEventFn('afterSave', callback);
      });
